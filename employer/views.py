...
class EmployeeRelation(g.CreateAPIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            dep_obj = Department.objects.get(id=data['department'])
            obj = EmployeeDepartment.objects.create(employee=emp_obj,department=dep_obj,
            employee_role=data['employee_role'],active=2)
            response = {'status': 2, 'message': "Created Successfully", 'data':
            {'id':obj.id,'employee':emp_obj.first_name,'departmet':dep_obj.department_name,
